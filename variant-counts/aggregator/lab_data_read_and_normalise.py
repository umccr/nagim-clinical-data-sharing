...
def retrieve(
    ...
    if csv:
        ...
        if key_or_path.endswith("/"):
            ...
            if isinstance(limit, int) and limit > 0:
                df = pl.scan_parquet(scan_wildcard, n_rows=limit).collect()
            else:
                df = pl.scan_parquet(scan_wildcard).collect()
        else:
            if isinstance(limit, int) and limit > 0:
                df = pl.read_parquet(scan_path, n_rows=limit)
            else:
    ...
    print(
        f"The estimate is that our retrieved data contains {round(df.estimated_size('mb'))} mb of data"
    )
