...
from common.vcf_aggregate_counts_vep_parquet_types import complete_record_schema
...
def main():
    ...
    with pl.Config() as cfg:
        ...
        vep_variants_df = pl.read_parquet(
            "./variant-counts-aggregate.parquet", schema=complete_record_schema
        )

        # filter to only variants in the gene of interest
        vep_variants_df = vep_variants_df.filter(
            pl.col("gene_id_set").list.contains(args.geneid)
        )

        # filter the counts we are interested in
        vep_variants_df = vep_variants_df.filter(pl.col("labs_contributing") > 1)
        ...
        vep_variants_df = (
            vep_variants_df.drop("allele_string", "phenotype_or_disease")
            .drop(
                "cdna_start",
                "cdna_end",
                "cds_start",
            ...
            .drop("hgvsp", "hgvsc", "codons", "protein_id")
            .drop("seq_region_name", "allele_num", "variant_allele", "somatic")
        )
